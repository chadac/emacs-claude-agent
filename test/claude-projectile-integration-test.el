...
;;; Commentary:
;; Real integration tests for claude-agent projectile support.
;; 
;; REQUIREMENTS:
...
;;
;; These tests verify that claude-agent correctly integrates with projectile
...
(defun claude-test--try-load-projectile ()
...
(if (claude-test--try-load-projectile)
...
(defmacro claude-test-require-projectile (&rest body)
  "Execute BODY only if projectile is available, otherwise skip test."
  `(if (featurep 'projectile)
       (progn ,@body)
     (ert-skip "Projectile not available - skipping test")))

(defmacro claude-test-with-projectile-preference (enabled &rest body)
  "Execute BODY with `claude-prefer-projectile-root' set to ENABLED."
  (declare (indent 1))
  `(let ((original-value claude-prefer-projectile-root))
     (unwind-protect
         (progn
           (setq claude-prefer-projectile-root ,enabled)
           ,@body)
       (setq claude-prefer-projectile-root original-value))))

(defun claude-test-create-directory-structure (base-dir structure)
...
(defun claude-test-init-git-repo (dir)
...
(ert-deftest claude-test-projectile-basic-integration ()
...
└── src/
  ...
  (claude-test-require-projectile
   (let ((project-dir (make-temp-file "claude-basic-projectile" t)))
    (unwind-protect
        (progn
          (claude-test-create-directory-structure project-dir
            '((".projectile" . "")
              ("src/" . nil)
              ("src/file.el" . "(message \"test\")")))
          
          (claude-test-with-projectile-preference t
...
(ert-deftest claude-test-projectile-vs-git-preference ()
...
└── child/
    ...
    └── file.txt"
  :tags '(:integration :projectile)
  (let ((parent-dir (make-temp-file "claude-preference-test" t)))
    (unwind-protect
        (let ((child-dir (expand-file-name "child" parent-dir)))
          (claude-test-create-directory-structure parent-dir
            '((".projectile" . "")
              ("child/" . nil)
              ("child/.git/" . nil)
              ("child/file.txt" . "test content")))
          (claude-test-init-git-repo child-dir)
          ...
          (let ((test-file (expand-file-name "child/file.txt" parent-dir))
            ...
            (cl-letf (((symbol-function 'buffer-file-name) 
              ...
              (claude-test-with-projectile-preference nil
...
(ert-deftest claude-test-projectile-package-json ()
...
└── src/
    └── app.js"
  :tags '(:integration :projectile)
  (let ((project-dir (make-temp-file "claude-package-json" t)))
    (unwind-protect
        (progn
          (claude-test-create-directory-structure project-dir
...
(ert-deftest claude-test-projectile-monorepo ()
  ...
  (let ((monorepo-dir (make-temp-file "claude-monorepo" t)))
    (unwind-protect
        (let ((backend-dir (expand-file-name "backend" monorepo-dir))
          ...
          (claude-test-init-git-repo backend-dir)
          (claude-test-init-git-repo frontend-dir)
...
(ert-deftest claude-test-projectile-nested-projects ()
...
└── inner/
    ...
    └── code/
        └── file.py"
  :tags '(:integration :projectile)
  (let ((outer-dir (make-temp-file "claude-nested" t)))
    (unwind-protect
        (let ((inner-dir (expand-file-name "inner" outer-dir)))
          (claude-test-create-directory-structure outer-dir
...
(ert-deftest claude-test-projectile-explicit-dir-parameter ()
...
└── subdir/
    └── file.txt"
  :tags '(:integration :projectile)
  (let ((project-dir (make-temp-file "claude-explicit" t)))
    (unwind-protect
        (let ((subdir (expand-file-name "subdir" project-dir)))
          ...
          (claude-test-with-projectile-preference t
            ...
            (let ((default-directory subdir))
              ...
              (let ((projectile-root (projectile-project-root))
                    (our-result (claude--project-root subdir)))
...
(ert-deftest claude-test-projectile-function-not-available ()
  "Test fallback when projectile function is unavailable."
  :tags '(:integration :projectile)
  (let ((test-dir (make-temp-file "claude-no-projectile" t)))
    (unwind-protect
        (progn
          (claude-test-create-directory-structure test-dir
            '((".git/" . nil)
              ("file.txt" . "test content")))
          (claude-test-init-git-repo test-dir)
...
(ert-deftest claude-test-projectile-throws-error ()
  "Test graceful fallback when projectile throws an error."
  :tags '(:integration :projectile)
  (let ((test-dir (make-temp-file "claude-projectile-error" t)))
    (unwind-protect
        (progn
          (claude-test-create-directory-structure test-dir
            '((".git/" . nil)
