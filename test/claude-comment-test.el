...
;; Author: Claude Code (extracted from claude-test.el)
;; Version: 0.1.0
;; Package-Requires: ((emacs "28.1") (ert "1.0"))

;;; Commentary:
;; Test suite for claude-comment.el using ERT (Emacs Lisp Regression Testing).
;; 
;; Test categories:
...
;; Add parent directory to load path to find claude-comment
(add-to-list 'load-path (file-name-directory (directory-file-name (file-name-directory load-file-name))))
(require 'claude-comment)

;;; Test Utilities

(defmacro claude-comment-test-with-temp-buffer (&rest body)
  "Execute BODY in a temporary buffer with claude-comment loaded.
...
(ert-deftest claude-test-comment-syntax-info-elisp ()
  "Test comment syntax information extraction in Emacs Lisp mode."
  :tags '(:unit :comment)
  (claude-comment-test-with-temp-buffer
    (emacs-lisp-mode)
    (let ((info (claude--comment-syntax-info)))
...
(ert-deftest claude-test-comment-syntax-info-c-mode ()
...
(ert-deftest claude-test-point-in-comment-detection ()
...
(ert-deftest claude-test-point-in-comment-mixed-line-c ()
...
(ert-deftest claude-test-point-in-comment-edge-case-line-boundary ()
...
(ert-deftest claude-test-point-in-comment-false-positive-prevention ()
...
(ert-deftest claude-test-comment-bounds-basic-single-line ()
  ...
  (claude-comment-test-with-temp-buffer
    ...
    (let ((bounds (claude--get-comment-bounds)))
      (should bounds)
    ...
    (should-not (claude--get-comment-bounds))))

(ert-deftest claude-test-comment-bounds-multi-line-block ()
...
(ert-deftest claude-test-comment-bounds-mixed-code-comment ()
...
(ert-deftest claude-test-comment-bounds-special-content ()
  "Test comment boundary detection with special keywords like TODO."
  :tags '(:unit :comment :bounds)
  (claude-comment-test-with-temp-buffer
    (emacs-lisp-mode)
    ;; Simulate the exact case from claude-agent.el:620
...
(ert-deftest claude-test-comment-bounds-c-style-multiline ()
...
(ert-deftest claude-test-comment-bounds-option-2-behavior ()
...
(ert-deftest claude-test-comment-bounds-option-2-no-before ()
...
(ert-deftest claude-test-comment-bounds-option-2-mixed-scenarios ()
...
(ert-deftest claude-test-comment-bounds-indentation-variations ()
...
(ert-deftest claude-test-comment-text-extraction ()
  ...
  (claude-comment-test-with-temp-buffer
    ...
    (let ((text (claude--extract-comment-text 1 (point-max))))
...
(ert-deftest claude-test-comment-text-extraction-multiline ()
